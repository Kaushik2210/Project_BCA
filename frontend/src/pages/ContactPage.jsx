// Import React hooks for refs and state management.
import React, { useRef, useState } from 'react';
// Import shared layout components.
import Navbar from '../components/Navbar';
import Footer from '../components/Footer';
// Import background image for the hero parallax effect.
import historyBg from '../assets/hero-church-epic.png';
// Import GSAP libraries for scroll-triggered reveal animations.
import gsap from 'gsap';
import { useGSAP } from '@gsap/react';
import { ScrollTrigger } from 'gsap/ScrollTrigger';
// Import react-icons for the contact information icons (Map, Phone, Mail).
...
// =========================================================================
// ContactPage — A public-facing page for visitors to send messages to the church.
// Features: Contact form with validation, GSAP reveal animations, contact info
// display, and an embedded Google Maps iframe.
// =========================================================================
const ContactPage = () => {
    // Ref for the main container — used as GSAP animation scope.
    const container = useRef();
    // Form data state — tracks the values of all three input fields.
    const [formData, setFormData] = useState({
        name: '',
        email: '',
        message: ''
    });
    // Status state — displays success message, error messages, or "Sending...".
    ...
    // ── GSAP Animations ──
    useGSAP(() => {
        // Hero Text Animation: Slide up from 100px below and fade in.
        gsap.fromTo(".hero-text",
            { y: 100, opacity: 0 },
            { y: 0, opacity: 1, duration: 1.5, ease: "power3.out", stagger: 0.2 }
        );

        // Contact Section Reveal: Each element with class .contact-reveal
        // slides up and fades in when the contact section scrolls into view.
        gsap.fromTo(".contact-reveal",
            { y: 50, opacity: 0 },
            {
                ...
                opacity: 1,
                duration: 1,
                stagger: 0.2,          // 200ms delay between each element's animation.
                ease: "power2.out",
                scrollTrigger: {
    ...
    // Generic input change handler using computed property name [name].
    ...
    const handleSubmit = async (e) => {
        ...
        // Destructure and trim the form values for validation.
        ...
        // Validation 2: Name must be at least 3 characters.
        if (nameStr.length < 3) {
            setStatus('Name must be at least 3 characters long.');
            return;
        }

        // Validation 3: Name should not contain numbers (regex \d matches digits).
        if (/\d/.test(nameStr)) {
            setStatus('Name should not contain any numbers.');
            return;
        }

        // Validation 4: Email must match a basic email regex pattern.
        ...
        // Validation 5: Message must be between 10 and 500 characters.
        if (msgStr.length < 10) {
        ...
        // Send the POST request to the backend /api/v1/contact endpoint.
        ...
        // On success: Show confirmation and reset form after 2 seconds.
    ...
    return (
        <>
            ...
            <div ref={container} className="bg-brand-beige text-brand-dark overflow-hidden min-h-screen">
                ...
                {/* ── Hero Section ── */}
                {/* Half-height hero with background image and centered title */}
                ...
                {/* Two-column grid: Left = contact details, Right = form */}
                        ...
                        <div className="contact-reveal bg-brand-dark/5 border border-brand-red/20 p-8 md:p-12 rounded-3xl backdrop-blur-sm">
                            <form onSubmit={handleSubmit} className="space-y-6">
                                ...
                                <div>
                                    ...
                                    ></textarea>
                                    {/* Dynamic character counter — turns red when limit exceeded */}
                ...
                {/* Shows the church's physical location using a Google Maps iframe */}
