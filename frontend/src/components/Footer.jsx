...
const Footer = () => {
  ...
  // Function to handle the form submission asynchronously to our Express backend
  const handleSubscribe = async (e) => {
    e.preventDefault(); // Prevents the default browser behavior of refreshing the page on form submit
    ...
    // Set UI to loading state so button shows spinner
    setStatus("loading");
    setMessage("");
    ...
    try {
      ...
      const response = await fetch(`${backendURL}/api/v1/newsletter/subscribe`, {
        ...
        body: JSON.stringify({ email }), // Sends the email state as JSON
      });
      const data = await response.json();

      // Ensure HTTP status is 200/201 ok AND our custom API success flag is true
      ...
      setStatus("error"); // Sets error if the network fails completely
  ...
  useGSAP(
    () => {
      ...
      // This targets all `.footer-char` elements generated by the <SplitText> component below
      gsap.from(".footer-char", {
        y: 50,
        opacity: 0,
        duration: 1,
        stagger: 0.05, // Rapid cascading delay between each letter fading up
        ease: "power3.out",
        scrollTrigger: {
          trigger: ".footer-title",
          start: "top 80%", // Animates when title is 80% down the screen
        },
      });
...
// Helper Component used to break text apart into individual <span> letters
// This makes it so we can animate each individual character with GSAP stagger (see above)
const SplitText = ({ children, className }) => {
  return (
    <span className={`inline-block ${className}`}>
      {/* Splits the string into array of chars and maps them to individual spans */}
      {children.split("").map((char, index) => (
        <span
          key={index} // React needs unique keys for arrays
          className="footer-char inline-block min-w-[0.2em] whitespace-pre" // Keeps spaces from collapsing
        >
          {char}
        </span>
      ))}
